...
export const translations = {
  ...
  'cc-grafana-info.addon-description': () => sanitize`This dashboard includes several graphs about a Clever Cloud application addon. <br> It provides first an overview panel containing server data as <strong> cpu, memory, disks and net </strong>. Only for <strong> MySQL, PostgreSQL, MongoDB and redis </strong> addon the database panel will contains data and provides information as <strong> connections count, queries or transactions, errors or deadlocks and tuples operations.<strong> `,
  'cc-grafana-info.addon-title': `Application add-on dashboard preview`,
  ...
  'cc-grafana-info.documentation-description': `This add-on is used to visualise Clever Cloud Metrics data. You can find the documentation and details about this service here`,
  ...
  'cc-grafana-info.error-link-doc': `Something went wrong while loading Grafana documentation link`,
  ...
  'cc-grafana-info.error-reseting': `Something went wrong while reseting Grafana dashboards`,
  ...
  'cc-grafana-info.loading-data': `Currently loading Grafana related data`,
  'cc-grafana-info.loading-title': `Loading`,
  'cc-grafana-info.main-title': `Metrics on Grafana`,
  'cc-grafana-info.organization-description': () => sanitize`This dashboard includes several graphs about a Clever Cloud organization. <br> It provides a graph resuming the amount of <strong> deployed applications runtimes and addons </strong>. It also provides a count per <strong> application type </strong> or <strong> runtime flavor </strong>. The <strong> deployment state graph </strong> indicates a new state for all new deployments done during the Grafana time interval. And finally on this dashboard you will retrieve <strong> global and specific links </strong> (sorted by requests count) to access an application runtime or addon dashboard.`,
  'cc-grafana-info.organization-title': `Organization dashboard preview`,
  'cc-grafana-info.reset': `Reset`,
  'cc-grafana-info.reset-description': `Reset all clever cloud template dashboard to their initial state`,
  'cc-grafana-info.reset-title': `Reset all dashboards`,
  'cc-grafana-info.runtime-description': () => sanitize`This dashboard includes an <strong> overview panel </strong> to get quick information about an application, as well as several panels about Clever Cloud application runtime metrics. <br> It provides graph resuming the state of <strong> the memory, disks, cpu and load usages or the network usages. </strong> For each of this type of metrics, you will retrieve usage graphs, gauges, as well as the result of a simple linear predication done on the interval time data collected. This prediction linear give as result the expected duration before the metrics is over 90%.`,
  'cc-grafana-info.runtime-title': `Application runtime dashboard preview`,
  //#endregion
